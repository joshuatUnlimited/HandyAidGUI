...
class ParsingMixin:
    ...
    def extract_transcript(self, raw_output, segments):
        ...
        for line in raw_output.splitlines():
            stripped = line.strip()
            if not stripped or stripped.startswith(("[", "main", "system", "loading", "backend")):
                continue
    ...
    def update_progress_from_line(self, line):
        if not self.current_audio_duration:
            return
        end_times = [float(m.group(1)) for m in re.finditer(r"\[(\d+(?:\.\d+)?)\]", line)]
        if end_times:
            progress = min(100.0, max(0.0, max(end_times) / self.current_audio_duration * 100.0))
